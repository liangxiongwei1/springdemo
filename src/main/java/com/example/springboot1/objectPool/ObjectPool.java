...
public abstract class ObjectPool<T> {
    ...
    protected Vector<PoolObject<T>> objects = null; // 存放对象池中对象的向量(PooledObject类型)
    ...
    public synchronized void createPool() {
        ...
        objects = new Vector<PoolObject<T>>();
        for (int i = 0; i < numObjects; i++) {
            objects.addElement(create());
        }
    }

    public abstract PoolObject<T> create();
    ...
    private T findFreeObject() {
        T obj = null;
        PoolObject<T> pObj = null;
        // 获得对象池向量中所有的对象
        Enumeration<PoolObject<T>> enumerate = objects.elements();
        // 遍历所有的对象，看是否有可用的对象
        while (enumerate.hasMoreElements()) {
            pObj = (PoolObject<T>) enumerate.nextElement();
